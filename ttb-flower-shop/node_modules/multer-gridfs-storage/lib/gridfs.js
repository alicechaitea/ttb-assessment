...
class GridFsStorage extends events_1.EventEmitter {
    ...
    fromMulterStream(readStream, request, file) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            console.log('Stream options:', streamOptions); // Logging stream options
    
            return new Promise((resolve, reject) => {
                const emitError = (streamError) => {
                    console.error('Stream error:', streamError); // Log the error
                    this.emit('streamError', streamError, streamOptions);
                    reject(streamError);
                };
                const emitFile = (f) => {
                    console.log('File stored:', f); // Log the stored file object
                    if (!f || !f._id) {
                        console.error('File does not have _id or is undefined:', f); // Log if _id is missing or file is undefined
                        return reject(new Error('File does not have _id or is undefined'));
                    }
                    const storedFile = {
                        id: f._id,
                        filename: f.filename,
                ...
                writeStream.on('finish', () => {
                    ...
                    bucket.find({ filename: streamOptions.filename }).toArray((err, files) => {
                        if (err || !files || files.length === 0) {
                            console.error('Error finding file or file not found:', err);
                            return emitError(new Error('Error finding file or file not found'));
                        }
                        emitFile(files[0]);
                    });
                });
                ...
                pump_1.default([readStream, writeStream], (err) => {
                    if (err) {
                        ...
                        console.log('Pump completed successfully');
                    }
                });
            });
        });
